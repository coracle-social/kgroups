 ...
 */
export function createSigningSession(config, keyPackage) {
    if (config.signerIndices.length < keyPackage.threshold) {
        throw new Error(`Not enough signers: need ${keyPackage.threshold}, got ${config.signerIndices.length}`);
    }
 ...
 */
export function generateSigningSessionId() {
 ...
 */
export function generateNonceCommitment(session) {
 ...
 */
export function processPublicNonce(session, nonce) {
 ...
 */
export function hasAllNonces(session) {
 ...
 */
export function generatePartialSignature(session) {
    if (!session.myCommitment) {
    ...
    const pnonces = session.config.signerIndices
        .map((idx) => session.publicNonces.get(idx))
        .filter((n) => n !== undefined);
    // Get the signing context
    const ctx = Lib.get_group_signing_ctx(session.keyPackage.groupPubkey, pnonces, session.config.message);
    // Get the commitment package for our share
    const commit = Lib.get_commit_pkg([session.myCommitment], session.keyPackage.share);
 ...
 */
export function processPartialSignature(session, partialSig) {
    if (!hasAllNonces(session)) {
    ...
    const isValid = Lib.verify_partial_sig(ctx, pnonce, partialSig.pubkey, partialSig.psig);
    if (!isValid) {
 ...
 */
export function hasAllPartialSignatures(session) {
 ...
 */
export function aggregateSignatures(session) {
    ...
    const isValid = Lib.verify_final_sig(Lib.get_group_key_context(session.keyPackage.groupPubkey), session.config.message, signature);
    if (!isValid) {
 ...
 */
export function signWithShares(shares, message, threshold) {
    ...
    const commits = selectedShares.map((s) => Lib.create_commit_pkg(s.share));
    // Get signing context
    ...
    const signature = Lib.combine_partial_sigs(ctx, psigs);
    // Verify
    const isValid = Lib.verify_final_sig(Lib.get_group_key_context(groupPubkey), message, signature);
    if (!isValid) {
 ...
 */
export function verifySignature(groupPubkey, message, signature) {
    try {
        return Lib.verify_final_sig(Lib.get_group_key_context(groupPubkey), message, signature);
    }
    catch {
        return false;
    }
