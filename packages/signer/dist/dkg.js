 ...
 */
export function createDKGSession(config) {
 ...
 */
export function generateRound1Package(session) {
 ...
 */
export function processRound1Package(session, pkg) {
    ...
    if (pkg.vssCommitments.length !== session.config.threshold) {
        throw new Error(`Invalid VSS commitments length: expected ${session.config.threshold}, got ${pkg.vssCommitments.length}`);
    }
 ...
 */
export function generateRound2Packages(session) {
    ...
    for (let toIdx = 1; toIdx <= session.config.maxSigners; toIdx++) {
        ...
        const conversationKey = nip44.v2.utils.getConversationKey(hexToBytes(mySecretKey), recipientPubkey);
 ...
 */
export function processRound2Package(session, pkg) {
    ...
    const conversationKey = nip44.v2.utils.getConversationKey(hexToBytes(session.config.mySecretKey), senderPubkey);
    ...
    const isValid = Lib.verify_share(senderRound1.vssCommitments, shareObj, session.config.threshold);
    if (!isValid) {
 ...
 */
export function finalizeDKG(session) {
    ...
    const aggregatedCommitments = aggregateVSSCommitments(session.round1Packages, session.config.threshold);
    const keyPackage = {
        share: finalShare,
 ...
 */
function aggregateFirstCommitments(round1Packages) {
 ...
 */
function aggregateVSSCommitments(round1Packages, threshold) {
    const result = [];
    // Sort packages by participant index for deterministic ordering
    const sortedPackages = Array.from(round1Packages.entries())
        .sort(([idxA], [idxB]) => idxA - idxB);
 ...
 */
export function createKeyGroupWithDealer(threshold, maxSigners, secrets = []) {
 ...
 */
export function createKeyPackageFromShare(share, groupPubkey, vssCommitments, threshold, maxSigners) {
    return {
        share,
        groupPubkey,
        vssCommitments,
 ...
 */
export function generateRefreshShares(keyPackage) {
    const { threshold, maxSigners, share } = keyPackage;
    // Generate refresh package using frost library
    const refreshPkg = Lib.gen_refresh_shares(share.idx, threshold, maxSigners);
    return {
        fromIdx: share.idx,
...
export function applyRefreshShares(currentKeyPackage, refreshPackages) {
    ...
    for (let i = 1; i < refreshPackages.length; i++) {
        aggregatedRefreshCommits = Lib.merge_share_commits(aggregatedRefreshCommits, refreshPackages[i].vssCommitments);
    }
    ...
    const newVssCommitments = [vssCommitments[0]]; // Keep the group pubkey
    for (let i = 1; i < threshold; i++) {
        // Merge the original commitment at index i with the aggregated refresh commitment at i-1
        const merged = Lib.merge_share_commits([vssCommitments[i]], [aggregatedRefreshCommits[i - 1]]);
 ...
 */
export function verifyRefreshedShare(keyPackage) {
    try {
        // Verify the share against the VSS commitments
        return Lib.verify_share(keyPackage.vssCommitments, keyPackage.share, keyPackage.threshold);
    }
    catch {
        return false;
    }
