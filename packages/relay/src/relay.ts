...
class EventStore {
  ...
  private byGroup: Map<GroupId, Set<EventId>> = new Map();
  private byAuthor: Map<Pubkey, Set<EventId>> = new Map();
  private byKind: Map<number, Set<EventId>> = new Map();
  private recentEventPrefixes: Set<string> = new Set();
  ...
  get(eventId: EventId): Event | undefined {
  ...
  query(filter: NostrFilter): Event[] {
    ...
    if (filter["#h"] && filter["#h"].length > 0) {
      const groupIds = new Set<EventId>();
    ...
    if (filter.authors && filter.authors.length > 0) {
      const authorIds = new Set<EventId>();
  ...
  hasRecentEventPrefix(prefix: string): boolean {
    return this.recentEventPrefixes.has(prefix);
  }

  getRecentEventPrefixes(): Set<string> {
...
function matchesFilter(event: Event, filter: NostrFilter): boolean {
...
export class Relay {
  ...
  private capabilityStore: CapabilityStore = new CapabilityStore();
  private groups: Map<GroupId, GroupMetadata> = new Map();
  private admins: Map<GroupId, GroupAdmin[]> = new Map();
  private members: Map<GroupId, Set<Pubkey>> = new Map();
   ...
   */
  private handleMessage(client: ClientState, message: ClientMessage): void {
    const [type] = message;
    ...
    switch (type) {
      case "EVENT":
        ...
        break;
      case "REQ":
        this.handleReq(client, message[1], message.slice(2) as NostrFilter[]);
        break;
      case "CLOSE":
   ...
   */
  private handleEvent(client: ClientState, event: Event): void {
   ...
   * Handle NIP-29 event
   */
  private handleNip29Event(client: ClientState, event: Event): void {
    const groupId = getGroupId(event);
    if (!groupId) {
    ...
    const validationOptions: Parameters<typeof validateNip29Event>[1] = {
   ...
   */
  private isGroupCreationEvent(event: Event): boolean {
  ...
  private checkEventAuthorization(
    ...
    let requiredCapability: CapabilityType | null = null;
   ...
   */
  private handleModerationEvent(client: ClientState, event: Event, groupId: GroupId): void {
   ...
   * Handle join request
   */
  private handleJoinRequest(client: ClientState, event: Event, groupId: GroupId): void {
    const group = this.groups.get(groupId);
    if (!group) {
      this.send(client, ["OK", event.id, false, "invalid: group not found"]);
      return;
   ...
   */
  private handleLeaveRequest(client: ClientState, event: Event, groupId: GroupId): void {
    const memberSet = this.members.get(groupId);
    if (memberSet) {
   ...
   */
  private handleCapabilityEvent(client: ClientState, event: Event): void {
    const groupId = getGroupId(event);
    if (!groupId) {
      this.send(client, ["OK", event.id, false, "invalid: missing h tag"]);
      return;
   ...
   * Handle REQ message
   */
  private handleReq(client: ClientState, subId: string, filters: NostrFilter[]): void {
   ...
   */
  private handleClose(client: ClientState, subId: string): void {
   ...
   * Handle AUTH message
   */
  private handleAuth(client: ClientState, event: Event): void {
    if (!verifyEvent(event)) {
      this.send(client, ["OK", event.id, false, "invalid: bad signature"]);
      return;
    }
   ...
   */
  private send(client: ClientState, message: RelayMessage): void {
   ...
   */
  private broadcastEvent(event: Event, groupId: GroupId): void {
    for (const client of this.clients) {
